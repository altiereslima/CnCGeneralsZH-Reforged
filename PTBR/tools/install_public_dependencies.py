...
def install_gamespy(repo: Path, source_override: Path|None):
    ...
    temp=parent/"GameSpy.__stage17_tmp"
    if temp.exists(): shutil.rmtree(temp)
    run([git,"clone","--no-checkout","--filter=blob:none",GAMESPY_REPO,str(temp)])
    run([git,"checkout",GAMESPY_COMMIT],cwd=temp)
    head=run([git,"rev-parse","HEAD"],cwd=temp).strip()
    if head.lower()!=GAMESPY_COMMIT.lower():
        raise RuntimeError(f"commit GameSpy inesperado: {head}")
    validate_gamespy(temp)
    if dst.exists(): shutil.rmtree(dst)
    shutil.move(str(temp),str(dst))
    # Do not carry Git metadata into the game source tree.
    gd=dst/".git"
    if gd.exists():
        if gd.is_dir(): shutil.rmtree(gd)
        else: gd.unlink()
