...
def main():
    ...
    if not (package/"payload/GeneralsMD/Code/Data/PortugueseBrazil/Generals.str").is_file():
    ...
    with tempfile.TemporaryDirectory(prefix="zh-ptbr-media-") as td:
        ...
        with zipfile.ZipFile(archive) as z:
            safe_extract(z,td)
        src=find_root(td)
        dst=package/"payload/GeneralsMD/Code/Data/PortugueseBrazil"
    ...
    missing=[x for x in MEDIA_FILES if not (package/"payload/GeneralsMD/Code/Data/PortugueseBrazil"/x).is_file()]
    if missing:
        raise RuntimeError("mídia ausente após instalação: "+", ".join(missing))
    print("PTBR MEDIA INSTALL PASS")
    print("SHA256:",got)
