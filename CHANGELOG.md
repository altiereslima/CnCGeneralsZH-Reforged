...
- Watching, every player's net worth is on screen under the strips drop-down, the way Dota shows it: richest first, the general's portrait, the number, and a bar measured against the richest player, green for one side and red for the other in a two-sided match and each player's own colour in a free for all. Net worth is the cash in hand plus what everything the player has standing would cost to build again, counted twice a second. Click the minus to fold it away.
- The number that list ranks by is yours to pick, as in Dota's game stats. Click its title and it opens into net worth, cash, income per minute, army value, kills and losses, the general's rank and power; the list re-sorts the moment you choose.
- Under it, a graph of the net worth lead since the first minute: green above the line while the first team is ahead, red below while the second is, with how far ahead the leader is right now. It appears in any match with two sides.
- An armies chart, closed until you open it: every player's six most expensive kinds of unit, with how many of each he has on the field.
