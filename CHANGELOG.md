...
- Watching, the strips can be switched off. A small "Strips" drop-down in the top left corner opens into three boxes - the production queue, the generals' promotions and the superweapon timers - and each box switches its strip the moment it is clicked and is remembered for the next match you watch. The message list moves down to start under it. Playing, there is no drop-down and every strip is up, whatever you chose in the stands. The drop-down is written as an HTML page under the game's Window folder: open it in a browser and the same boxes are there, and a box added to it for another on/off setting shows up in the next match you watch.
